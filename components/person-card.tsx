...
function PersonCard({
  ...
  return (
    ...
    >
      ...
      <div
        className={`${
          size == "small" ? "md:items-center md:text-center" : "md:text-left md:mt-12 "
        } flex items-center flex-col md:items-start`}
      >
        <h5
          className={`${size == "small" ? "text-xl" : "text-4xl"} font-bold`}
        >
          {name}
        </h5>
        <p className={`${size == "small" ? "hidden" : ""} sm:w-2/3`}>{description}</p>
      </div>
    </div>
  );
}
